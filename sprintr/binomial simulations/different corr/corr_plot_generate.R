...
# Define the structures, methods, and correlations
structures <- c("hier_strong", "hier_weak", "mix", "anti")
methods <- c("SIS", "APL", "MEL", "glinternet", "hierNet", "hierFabs", "RAMP", "sprinterT")
...
for (structure in structures) {
  ...
  for (method in methods) {
    ...
    for (corr in 1:length(correlation)) {
      results[[structure]]$dev[[method]] <- append(results[[structure]]$dev[[method]], mean(eval.list[[corr]][[dev_method_name]]))
      results[[structure]]$auc[[method]] <- append(results[[structure]]$auc[[method]], mean(eval.list[[corr]][[auc_method_name]]))
    }
  }
}

# Debug: Print the results to ensure they contain the expected data
print(results)
...
pdf(file="logistic_correlation_plot.pdf", width=30, height=16.5)
...
for (metric in c("dev", "auc")) {
  for (structure in structures) {
    ...
    xlab <- ifelse(metric == "auc", TeX('\\rho$'), "")
    ...
    if (metric == "dev") {
      if (structure == "hier_strong") {
        ...
        plot(1, type="n", ylab="", xlim=c(min(correlation), max(correlation)), ylim=yrange, cex.axis=5, cex.lab=5,
        ...
        plot(1, type="n", xlim=c(min(correlation), max(correlation)), ylim=yrange, cex.axis=5, main=main, cex.main=5,
             font.main=1, xaxt="n", yaxt="n", font.lab=2)
        box(lwd=3)
        axis(2, las=0, cex.axis=4.2, padj=0.5)
      }
    } else if (metric == "auc") {
      if (structure == "hier_strong") {
        ...
        plot(1, type="n", xlab="", ylab="", xlim=c(min(correlation), max(correlation)), ylim=yrange, cex.axis=5,
             cex.lab=5, xaxt="n", yaxt="n", font.lab=2)
        box(lwd=3)
        mtext(side=2, line=6, ylab, cex=3.4)
        mtext(side=1, line=8, xlab, cex=3.3)
        axis(1, at=correlation, labels=as.character(correlation), cex.axis=4.5, padj=0.2)
        axis(2, las=0, cex.axis=4.2, padj=0.5)
      } else {
        par(mar = c(7, 4, 3.5, 2), mgp = c(5.8, 2.3, 0))
        plot(1, type="n", xlab="", xlim=c(min(correlation), max(correlation)), ylim=yrange, cex.axis=5,
             cex.lab=5, xaxt="n", yaxt="n", font.lab=2)
        box(lwd=3)
        mtext(side=1, line=8, xlab, cex=3.3)
        axis(1, at=correlation, labels=as.character(correlation), cex.axis=4.5, padj=0.2)
    ...
    for (i in 1:length(methods)) {
      points(correlation, data_all[[i]], col=col[i], pch=pch[i], cex=4, lwd=3)
      lines(correlation, data_all[[i]], col=col[i], cex=4, lwd=5, lty=lty[i])
    }
  }
}
