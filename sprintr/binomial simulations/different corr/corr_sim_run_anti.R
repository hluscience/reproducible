...
rm(list=ls())
source("corr_binomial_parm_generate.R") # Source the parameter generation script
...
p <- 200 # Number of main effects
stru <- "anti" # Structure type of the simulated data. Options are "anti", "mix", "hier_weak" and "hier_strong".
correlation <- c(0, 0.2, 0.4, 0.6, 0.8) # Different main vs. interaction effects signal strength
method <- c("sprinterT", "glinternet", "MEL", "APL", "SIS", "RAMP", "hierFabs", "hierNet") # Methods to be evaluated
...
for (i in seq_along(correlation)){
  ...
  parm <- parm_gen(stru = stru, n = n, p = p, coef_inter = 2, ratio = 0.5, corr_gen = correlation[i])
  ...
  for(j in 1:num_sim){
    ...
    message(sprintf("Simulation %d completed for correlation %s in structure %s", j, correlation[i], stru))
    end.time.j <- Sys.time()
  ...
  message(sprintf("Correlation %s completed for structure %s", correlation[i], stru))
  end.time.i <- Sys.time()
  time <- end.time.i - start.time.i
  message(sprintf("Time taken for correlation %s: %s", correlation[i], time))
