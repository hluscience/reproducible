...
increasing_n_method_run_eval<- function(x, y, x_test, y_test, pure_inter) {
  ...
  if(nrow(sprintr_idx) == 1)
    design <- myscale(matrix(x_scale[, sprintr_idx[, 1]] * x_scale[, sprintr_idx[, 2]], ncol = 1))
  else
    design <- myscale(x_scale[, sprintr_idx[, 1]] * x_scale[, sprintr_idx[, 2]])
  ...
  design <- cbind(x_scale, design)

  # The Third Step:
  # in Step3, we fit the response with offset being mu from Step 1
  lambda3 <- get_lambda(x = design, y = y, family = "binomial", nlam = 100, offset = main_sample_quantity, lam_min_ratio = 0.01)

  # in Step 3 we no longer fit intercept, which has been captured in Step 1
  fit <- glmnet::cv.glmnet(x = design, y = y, family = "binomial",
                           lambda = lambda3,
                           offset = main_sample_quantity,
                           intercept = FALSE,
                           standardize = FALSE,
                           type.measure = "deviance")
  ...
  beta <- as.numeric(fit$glmnet.fit$beta[, ibest])
  beta <- beta / col_sd3
  a0 <- fit$glmnet.fit$a0[ibest] + as.numeric(- crossprod(col_mean3, beta))
  ...
  idx <- rbind(cbind(rep(0, p), seq(p), rep(0, p)), sprintr_idx)
  compact <- cbind(idx[which(coef != 0), 1:2, drop = FALSE], coef[coef != 0])
  colnames(compact) <- c("index_1", "index_2", "coefficient")
  a0 <- step1$a0 +  step3$a0

  sprintr_result <- list(p = p,
                         compact = compact,
                         a0 = a0)

  # Calculate prediction error
  mu_test <- sprintr::predict.other(sprintr_result, x_test) # predictions
...
increasing_corr_run <- function(x, y) {
  ...
  sprintr_idx <- sprintr::screen_cpp_MMLE_binomial(x = x_scale, y = y, offset = offset, num_keep = (p^2+p)/2, square = FALSE)
  sprintr_idx <- sprintr_idx[!is.na(sprintr_idx[, 3]), , drop = FALSE]
  sprintr_idx <- sprintr_idx[order(sprintr_idx[, 3], decreasing = TRUE), , drop = FALSE]
  colnames(sprintr_idx) <- c("index_1", "index_2", "score")
...
sprinter_select_inter <- function(x, y, family = "gaussian", nlev = 1, type.measure = "deviance", square = FALSE, num_keep = NULL, lambda1 = NULL, lambda3 = NULL, nlam1 = 10, nlam3 = 100, lam_min_ratio = ifelse(nrow(x) < ncol(x), 0.01, 1e-04)){
  ...
  # in Step3, we fit the response with offset being mu from Step 1
  if(is.null(lambda3)){
    lambda3 <- get_lambda(x = design, y = y, family = family,
                               nlam = nlam3, offset = mu,
  ...
  # output from step 3
  step3 <- list()
  step3$beta <- as.matrix(theta / col_sd3)
  step3$a0 <- a0 + as.numeric(- crossprod(col_mean3, step3$beta))
  ...
  if(square){
    ...
    idx <- rbind(cbind(rep(0, p), seq(p), rep(0, p)), idx)
  }
...
oracle_select_inter <- function(x, y, family = "gaussian", num_keep = NULL, type.measure = "deviance"){
  ...
  # construct design matrix of selected interactions
  if(nrow(top_interactions) == 1)
    design <- myscale(matrix(x[, top_interactions[, 1]] * x[, top_interactions[, 2]], ncol = 1))
  else
    design <- myscale(x[, top_interactions[, 1]] * x[, top_interactions[, 2]])
  ...
  # output from step 3
  coef <- as.matrix(theta / col_sd3)
  a0 <- a0 + as.numeric(- crossprod(col_mean3, coef))

  idx <- rbind(cbind(rep(0, p), seq(p), rep(0, p)), top_interactions)
  compact <- cbind(idx[which(coef != 0), 1:2, drop = FALSE], coef[coef != 0])
  colnames(compact) <- c("index_1", "index_2", "coefficient")

  out <- list(idx = top_interactions,
              p = p,
              compact = compact,
              a0 = a0)
  return(out)
...
compare_with_oracle_method_run_eval <- function(x, y, x_test, y_test, pure_inter_idx){

  # Run sprinter method
  start.time <- Sys.time()
  sprinter_result <- sprinter_select_inter(x = x, y = y, family = "binomial", square = FALSE, nlam1 = 100, nlam3 = 100, lam_min_ratio = 0.01)
  end.time <- Sys.time()
  ...
  # Calculate TPR and FPR for sprinter method
  TPR_FPR <- TPR_FPR_get(ncol(x), sprinter_result$idx, pure_inter_idx)
  eval_vec$FPR_sprinter <- append(eval_vec$FPR_sprinter, TPR_FPR$FPR)
  eval_vec$TPR_sprinter <- append(eval_vec$TPR_sprinter, TPR_FPR$TPR)
  cat("sprinter Done", fill = TRUE)

  # Calculate prediction error
  mu_test <- sprintr::predict.other(sprinter_result, x_test) # predictions
  dev <- compute_deviance_logistic(y_test, mu_test) # deviance
  eval_vec$dev_sprinter <- append(eval_vec$dev_sprinter, dev)

  # Run oracle method
  start.time <- Sys.time()
  oracle_result <- oracle_select_inter(x = x, y = y, family = "binomial")
  end.time <- Sys.time()
  time_spend <- as.numeric(difftime(end.time, start.time, units = "secs"))
  eval_vec$time_oracle <- append(eval_vec$time_oracle, time_spend)

  # Calculate TPR and FPR for oracle method
  TPR_FPR <- TPR_FPR_get(ncol(x), oracle_result$idx, pure_inter_idx)
  ...
  # Calculate prediction error
  mu_test <- sprintr::predict.other(oracle_result, x_test) # predictions
  dev <- compute_deviance_logistic(y_test, mu_test) # deviance
  eval_vec$dev_oracle <- append(eval_vec$dev_oracle, dev)

  return(eval_vec)
}

# Function to generate test data for identifying pure interactions
theory_proof_test_data_gen <- function(n, p, compact, corr_gen = 0){
