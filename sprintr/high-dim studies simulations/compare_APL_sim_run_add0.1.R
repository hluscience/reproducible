...
eta_seq <- c(0.1)  # Sequence of eta values to control the relative penalty between main effects and interactions
...
for (i in seq_along(p_seq)){
  ...
  message(sprintf("dev for APL_0.1: %s", mean_dev["dev_APL_0.1"]))
  message(sprintf("auc for glinternet: %s", mean_auc["auc_glinternet"]))
  message(sprintf("auc for sprinter: %s", mean_auc["auc_sprinter"]))
  message(sprintf("auc for APL_0.1: %s", mean_auc["auc_APL_0.1"]))
}

# Save the evaluation results to disk
saveRDS(eval_list, file = "eval_list_add0.1.rds")
